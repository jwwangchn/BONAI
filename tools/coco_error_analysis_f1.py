    ...
    for i in range(len(areaNames)):
        ...
        for k in range(len(types)):
            ...
            for count in range(len(ps_curve[k + 1])):
                psarray[count] = 2 * rs[count] * psarray[count] / (rs[count] + psarray[count] + 1e-6)
            ax.plot(rs, psarray , color=[0, 0, 0], linewidth=0.5)
            maxf1precision=max(psarray)
            inds=psarray.argmax()
            maxf1recall=rs[inds]
            input = areaNames[i] +' '+types[k] + ': ' + 'precision: '+ str(maxf1precision)+', recall:' +str(maxf1recall)+'\n'
...
def analyze_results(res_file, ann_file, res_types, out_dir):
    ...
    for res_type in res_types:
        ...
        cocoEval.params.imgIds = imgIds
        cocoEval.params.iouThrs = [.75, .5, .3]
        cocoEval.params.maxDets = [1500]
        cocoEval.evaluate()
        cocoEval.accumulate()
